...
const {
  editTaskSchema,
  createTaskSchema
} = require('../middleware/validation/schemas/task')

const router = express.Router()

router
  .route('/:projectId/tasks/all')
  .get(verifyToken(), TasksController.getProjectTasks)
...
router
  ...
  .get(verifyToken(), TasksController.getTaskById)
  .patch(
    verifyToken(),
    validateBody(editTaskSchema),
    TasksController.editTaskById
  )
  .delete(verifyToken(), TasksController.deleteTaskById)
