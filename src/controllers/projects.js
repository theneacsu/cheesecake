...
async function createNewProject (req, res, next) {
  ...
  try {
    ...
    const takenProjectTitle = user.projects.find(
      prj => prj.title === projectData.title
    )

    if (takenProjectTitle) {
      return res
        .status(403)
        .json({ projectTitleTaken: true, projectTitle: projectData.title })
    }
    ...
    const displayedProject = await Project.findById(savedProject.id).populate(
      'tasks'
    )

    return res
      .status(201)
      .json({ projectCreated: true, project: displayedProject })
  } catch (err) {
    next(err)
  }
}
...
async function getAllUserProjects (req, res, next) {
  ...
  try {
    ...
    if (user) {
      return res
        .status(200)
        .json({ userId, email: user.email, projects: user.projects })
    }
...
async function editProjectById (req, res, next) {
  ...
  try {
    ...
    if (project) {
      ...
      return res
        .status(200)
        .json({ updated: true, projectId, project: updatedProject })
    }
...
async function deleteProjectById (req, res, next) {
  ...
  try {
    ...
    return res
      .status(403)
      .json({ projectDeleted: false, projectNotFoud: true, projectId })
  } catch (err) {
    next(err)
  }
}
